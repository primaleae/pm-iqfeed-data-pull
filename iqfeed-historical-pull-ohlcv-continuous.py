...
def pull_data(symbol, exchange_security, timeframe, output_dir):
    ...
    if timeframe == "minute":
        ...
        maxDataPoints = 3  # should be all the way to 1900 and then some
        # weekly data message
        message = f"HWX,{symbol},{maxDataPoints},{dataDirection}\n"
    elif timeframe == "tick":
        # # tick start end
        startDate = "20221224 000000"
        endDate = "20221231 235959"
        # tick data message
...
if __name__ == "__main__":
    ...
    timeframe_list = ["weekly", "daily", "minute", "tick"]
    # timeframe_list = ["tick"]
    # timeframe_list = ["daily", "weekly"]
    ...
    overwrite = True
    ignore_symbols_in_path = False
    discard_ignored_symbol_list = False
    try_again_for_small_files = False
    ...
    #     need_to_check_again_list = pd.read_csv(
    #         need_to_check_again_path)["Symbol"].tolist()
    # else:
    #     with open(need_to_check_again_path, "w", encoding="utf-8") as f:
    #         f.write("Symbol\n")

    # if Path.exists(no_data_weekly_path):
    #     weekly_no_data_symbols_list = pd.read_csv(
    #         no_data_weekly_path)["Symbol"].tolist()
    # else:
    #     with open(no_data_weekly_path, "w", encoding="utf-8") as f:
    #         f.write("Symbol\n")
    # if Path.exists(unauthorized_weekly_path):
    #     weekly_unauthorized_symbols_list = pd.read_csv(
    #         unauthorized_weekly_path)["Symbol"].tolist()
    # else:
    #     with open(unauthorized_weekly_path, "w", encoding="utf-8") as f:
    #         f.write("Symbol\n")
    # weekly_ignore_symbol_list = weekly_no_data_symbols_list + \
    #     weekly_unauthorized_symbols_list

    # if Path.exists(no_data_daily_path):
    #     daily_no_data_symbols_list = pd.read_csv(
    #         no_data_daily_path)["Symbol"].tolist()
    # else:
    #     with open(no_data_daily_path, "w", encoding="utf-8") as f:
    #         f.write("Symbol\n")
    # if Path.exists(unauthorized_daily_path):
    #     daily_unauthorized_symbols_list = pd.read_csv(
    #         unauthorized_daily_path)["Symbol"].tolist()
    # else:
    #     with open(unauthorized_daily_path, "w", encoding="utf-8") as f:
    #         f.write("Symbol\n")
    # daily_ignore_symbol_list = daily_no_data_symbols_list + \
    #     daily_unauthorized_symbols_list

    # if Path.exists(no_data_minute_or_tick_path):
    #     minute_or_tick_no_data_symbols_list = pd.read_csv(
    #         no_data_minute_or_tick_path)["Symbol"].tolist()
    # else:
    ...
    #         unauthorized_minute_or_tick_path)["Symbol"].tolist()
    # else:
    #     with open(unauthorized_minute_or_tick_path, "w", encoding="utf-8") as f:
    #         f.write("Symbol\n")
    # minute_or_tick_ignore_symbol_list = minute_or_tick_no_data_symbols_list + \
    #     minute_or_tick_unauthorized_symbols_list
    ...
    have_symbols_for_weekly_path = Path(".../symbol_data_exists_weekly.csv")

    df = pd.read_csv(have_symbols_for_weekly_path, header=None)
    have_symbols_for_weekly_list = frozenset(df[0].to_list())

    have_symbols_for_daily_path = Path(".../symbol_data_exists_daily.csv")

    df_temp = pd.read_csv(have_symbols_for_daily_path, header=None)
    have_symbols_for_daily_list = frozenset(df_temp[0].to_list())

    have_symbols_for_minute_path = Path(".../symbol_data_exists_minute.csv")

    df_temp = pd.read_csv(have_symbols_for_minute_path, header=None)
    have_symbols_for_minute_list = frozenset(df_temp[0].to_list())

    have_symbols_for_tick_path = Path(".../symbol_data_exists_tick.csv")

    df = pd.read_csv(have_symbols_for_tick_path, header=None)
    have_symbols_for_tick_list = frozenset(df[0].to_list())
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=50) as executor:
        ...
        for timeframe in timeframe_list:
            ...
            if timeframe == "minute":
                # minute data message
                symbols_to_check = have_symbols_for_minute_list
            elif timeframe == "tick":
                # daily data message
                symbols_to_check = have_symbols_for_tick_list
            elif timeframe == "daily":
                # weekly data message
                symbols_to_check = have_symbols_for_daily_list
            elif timeframe == "weekly":
                # tick data message
                symbols_to_check = have_symbols_for_weekly_list
            ...
            for exchange_security in exchange_security_type_list:
                ...
                if try_again_for_small_files and overwrite:
                    if Path.exists(output_dir_path):
                        files_in_dir = os.listdir(output_dir_path)
                        for file in files_in_dir:
                            if os.path.getsize(output_dir_path / file) > 25000:
                                symbol_data_already_exists_list += [
                                    file.replace(".csv", "")
                                ]
                        print(len(files_in_dir), len(symbol_data_already_exists_list))
                    else:
                        continue
                for chunk in exchange_security_dict[exchange_security]:
                    ...
                    for symbol in chunk:
                        if symbol not in symbols_to_check:
                            continue
                        # if timeframe in ("minute", "tick"):
                        #     if symbol not in have_symbols_for_minute_list:
                        #         continue
                        # if not firstloop:
                        #     continue
                        # symbol = "@ES#"
                    ...
                    if skipped != 0:
                        print(f"Skipped {skipped} symbols for {exchange} - {security}")
                    if already_exists != 0:
                        print(
                    ...
                    for result in concurrent.futures.as_completed(results):
                        try:
                            ...
                            #     if timeframe in ("minute"): # don't necessarily want to exclude symbols that have minute but not tick
                            #         minute_or_tick_no_data_count += 1
                            #         with minute_manager_lock:
                            #             with open(no_data_minute_or_tick_path, "a", encoding="utf-8") as f:
                            #                 f.write(f"{symbol}\n")
                            #     elif timeframe == "daily":
                            #         daily_no_data_count += 1
                            #         with daily_manager_lock:
                            #             with open(no_data_daily_path, "a", encoding="utf-8") as f:
                            #                 f.write(f"{symbol}\n")
                            #     elif timeframe == "weekly":
                            #         weekly_no_data_count += 1
                            #         with weekly_manager_lock:
                            #             with open(no_data_weekly_path, "a", encoding="utf-8") as f:
                            #                 f.write(f"{symbol}\n")
                            # elif "unauthorized" in error_message:
                            #     if timeframe in ("minute", "tick"):
                            #         minute_or_tick_unauthorized_count += 1
                            #         with minute_manager_lock:
                            #             with open(unauthorized_minute_or_tick_path, "a", encoding="utf-8") as f:
                            #                 f.write(f"{symbol}\n")
                            #     elif timeframe == "daily":
                            #         daily_unauthorized_count += 1
                            #         with daily_manager_lock:
                            #             with open(unauthorized_daily_path, "a", encoding="utf-8") as f:
                            #                 f.write(f"{symbol}\n")
                            #     elif timeframe == "weekly":
                            #         weekly_unauthorized_count += 1
                            #         with weekly_manager_lock:
                            #             with open(unauthorized_weekly_path, "a", encoding="utf-8") as f:
                            #                 f.write(f"{symbol}\n")
                            # error_types_list = [
                            #     "socket",
                            #     "too many requests",
                            ...
                            #         print(f"{timeframe} - {symbol} - {error} in {error_message}, writing to check again", file=sys.stderr)
                            #         with need_to_check_again_manager_lock:
                            #             with open(need_to_check_again_path, "a", encoding="utf-8") as f:
                            #                 f.write(f"{symbol}\n")
    ...
    # subtract_from_count = minute_or_tick_no_data_count + minute_or_tick_unauthorized_count \
    #     + daily_no_data_count + daily_unauthorized_count \
    #     + weekly_no_data_count + weekly_unauthorized_count
